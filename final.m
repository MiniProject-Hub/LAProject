%% watermark an image into an audio file
% input:   host(audio file)
%          wm(watermark image)
% output:  host_new(watermarked audio file) 
...
audiowrite('test_new.wav', host_new, f)     % save watermarked host audio
% host      = uint8(255*(host + 0.5));  % double [-0.5 +0.5] to 'uint8' [0 255]
figure(3)
subplot(2,1,1)
plot(host_new)
